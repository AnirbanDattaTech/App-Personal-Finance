...
# MODIFICATIONS: Updated AssistantOutput model to include sql_query instead of sql_results_str.
...
# ASSUMPTION: The 'finance_assistant_graph' runnable's final output (AgentState)
# contains a field named 'sql_query' that LangServe can map to the AssistantOutput model.
# If the graph output state doesn't have 'sql_query', the graph itself needs updating.
